...
using BlockChain.BinaryOptions.Contract.IChainlinkPrice.ContractDefinition;

namespace BlockChain.BinaryOptions.Contract.IChainlinkPrice
{
    public partial class IChainlinkPriceService
    {
        public static Task<TransactionReceipt> DeployContractAndWaitForReceiptAsync(Nethereum.Web3.Web3 web3, IChainlinkPriceDeployment iChainlinkPriceDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            return web3.Eth.GetContractDeploymentHandler<IChainlinkPriceDeployment>().SendRequestAndWaitForReceiptAsync(iChainlinkPriceDeployment, cancellationTokenSource);
        }

        public static Task<string> DeployContractAsync(Nethereum.Web3.Web3 web3, IChainlinkPriceDeployment iChainlinkPriceDeployment)
        {
            return web3.Eth.GetContractDeploymentHandler<IChainlinkPriceDeployment>().SendRequestAsync(iChainlinkPriceDeployment);
        }

        public static async Task<IChainlinkPriceService> DeployContractAndGetServiceAsync(Nethereum.Web3.Web3 web3, IChainlinkPriceDeployment iChainlinkPriceDeployment, CancellationTokenSource cancellationTokenSource = null)
        {
            var receipt = await DeployContractAndWaitForReceiptAsync(web3, iChainlinkPriceDeployment, cancellationTokenSource);
            return new IChainlinkPriceService(web3, receipt.ContractAddress);
        }
        ...
        public IChainlinkPriceService(Nethereum.Web3.Web3 web3, string contractAddress)
        {
            Web3 = web3;
            ContractHandler = web3.Eth.GetContractHandler(contractAddress);
        }

        public IChainlinkPriceService(Nethereum.Web3.IWeb3 web3, string contractAddress)
        {
            Web3 = web3;
            ContractHandler = web3.Eth.GetContractHandler(contractAddress);
        }
        ...
        public Task<GetRoundPriceOutputDTO> GetRoundPriceQueryAsync(string aggregator, BigInteger time, BlockParameter blockParameter = null)
        {
            var getRoundPriceFunction = new GetRoundPriceFunction();
                getRoundPriceFunction.Aggregator = aggregator;
                getRoundPriceFunction.Time = time;
